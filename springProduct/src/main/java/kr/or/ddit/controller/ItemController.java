...
public class ItemController {
   ...
   private String uploadPath = "C:\\Users\\PC-10\\Desktop\\springProduct\\src\\main\\webapp\\resources\\upload"; 
   ...
   public String uploadFormAction(MultipartFile[] uploadFile) {
      ...
      for(MultipartFile multipartFile : uploadFile) {
         ...
         // File 객체 설계(복사할 대상 결로(서버쪽), 파일명(UUID 가 붙은))
   ...
   public boolean checkImageType(File file) {
	    ...
	    */
//	   		MIM 타입을 통해 이미지 여부 확인
//	   		file.toPath() : 파일 객체를 path 객체로 변환
	   String contentType;
	try {
		contentType = Files.probeContentType(file.toPath());
		log.info(contentType);
		//MIME 타입 정보가 image로 시작하는지에 여부를 return
